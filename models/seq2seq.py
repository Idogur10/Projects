...
class Seq2SeqLSTM(nn.Module):
    ...
    def __init__(self, encoder, decoder, horizon, delta_t, pos_mean, pos_std, scaler_vel):
        ...
        # Register Buffers for Normalization (velocity)
        ...
        # Acceleration has its own scale (indices 3:6 of the vel_scaler which covers cols 3-8)
        self.register_buffer('acc_mean', torch.tensor(scaler_vel.mean_[3:6], dtype=torch.float32))
        self.register_buffer('acc_std', torch.tensor(scaler_vel.scale_[3:6], dtype=torch.float32))
    ...
    def forward(self, input_batch, target_seq=None, teacher_forcing_ratio=0.5):
        ...
        current_pos = (input_batch[:, -1, :3] * self.pos_std) + self.pos_mean
        ...
        for t in range(self.horizon):
            ...
            # 1. Normalize Position (Standardize)
            norm_pos = (current_pos - self.pos_mean) / self.pos_std

            # 2. Normalize Velocity (Standard Scaler)
            norm_vel = (current_vel - self.scaler_mean) / self.scaler_std

            # 3. Normalize Acceleration (using acceleration's own scale)
            norm_acc = (current_acc - self.acc_mean) / self.acc_std
            ...
            pred_acc_real = (pred_raw * self.acc_std) + self.acc_mean
...
class Seq2SeqDaVinciNet(nn.Module):
    ...
    def __init__(self, input_size, hidden_size, seq_len, horizon, delta_t, pos_mean, pos_std, scaler_vel):
...
class Seq2SeqDaVinciNetVelAcc(nn.Module):
    """
    DaVinciNet variant where the decoder predicts velocity AND acceleration.

    Instead of predicting only acceleration and using Velocity Verlet,
    the network predicts both v_next and a_next. Jerk is derived as
    (a_next - a_current) / delta_t, and position is updated via
    3rd-order Taylor expansion:
        pos = pos + v*dt + 0.5*a*dt^2 + (1/6)*jerk*dt^3

    Output per timestep: (pos, vel, acc, jerk) = 12 dims.
    """

    def __init__(self, input_size, hidden_size, seq_len, horizon, delta_t, pos_mean, pos_std, scaler_vel):
        super(Seq2SeqDaVinciNetVelAcc, self).__init__()
        self.hidden_size = hidden_size
        self.seq_len = seq_len
        self.horizon = horizon
        self.delta_t = delta_t
        ...
        # Decoder LSTM cell
        # Input: physics state (9: pos+vel+acc) + context vector (hidden_size)
        self.decoder_cell = nn.LSTMCell(9 + hidden_size, hidden_size)

        # Output layer: predicts velocity + acceleration (6D)
        self.fc_out = nn.Linear(hidden_size + hidden_size, 6)  # decoder_hidden + context
    ...
    def forward(self, input_batch, target_seq=None, teacher_forcing_ratio=0.5):
        ...
        for t in range(self.horizon):
            ...
            # --- D. PREDICT VELOCITY + ACCELERATION ---
            decoder_context = torch.cat((d, context), dim=1)
            pred_raw = self.fc_out(decoder_context)  # (batch, 6)

            # Un-normalize: first 3 = velocity, last 3 = acceleration
            pred_vel = (pred_raw[:, :3] * self.scaler_std) + self.scaler_mean
            pred_acc = (pred_raw[:, 3:6] * self.acc_std) + self.acc_mean

            # --- E. PHYSICS UPDATE (3rd-order Taylor) ---
            # Jerk = (a_next - a_current) / dt
            jerk = (pred_acc - current_acc) / self.delta_t

            # Position via 3rd-order Taylor expansion
            dt = self.delta_t
            pos_predicted = (current_pos
                             + current_vel * dt
                             + 0.5 * current_acc * dt ** 2
                             + (1.0 / 6.0) * jerk * dt ** 3)

            # Update state
            current_vel = pred_vel
            current_acc = pred_acc

            # --- F. STORE (pos, vel, acc, jerk) = 12 dims ---
            output_save = torch.cat((pos_predicted, current_vel, current_acc, jerk), dim=1)
            path_vec.append(output_save.unsqueeze(1))
